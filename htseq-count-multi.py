...
'''
    Note: 
        ...
        Counts are returned in separate f.txt iles.
        Reads are returned as a single file with each read containing new flags: GF:Z, [EF:Z, XF:Z, XG:Z, XH:Z, ...]
    Modified_by: 
            ...
            for p in range(0, len(feature_type)):
                try:
                    ...
                    if fs[p] is None or len(fs[p]) == 0:
                        ...
                        samtagvalue[p] = "__ambiguous[" + '+'.join(fs[p]) + "]"
                        ambiguous[p] += 1
                    else:
